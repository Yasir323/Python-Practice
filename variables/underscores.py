...
2. Multiple time we do not want return values at that time assign those values to
...
means you shouldn’t access this method because it’s not part of the API.
    Name prefix by underscore is treated as non-public. If specify from Import * all
the name starts with _ will not import. Python does not specify truly private
so this ones can be call directly from other modules if it is specified in __all__,
